...
async function uploadFeaturedImage(keyword, auth) {
  // Try multiple free image sources
  const sources = [
    `https://picsum.photos/seed/${encodeURIComponent(keyword)}/1200/630`,
    `https://loremflickr.com/1200/630/${encodeURIComponent(keyword + ',politics')}`,
  ...
  for (const imageUrl of sources) {
    try {
      ...
      console.log(`🖼️  Image uploaded: ${uploadRes.data.source_url}`);
      return uploadRes.data.id;
    } catch (e) {
      console.warn(`⚠️ Image source failed (${imageUrl}): ${e.message}`);
    }
  }

  console.warn('⚠️ All image sources failed — posting without featured image.');
  return null;
}
...
// ── MAIN CYCLE — post one article per category ────────────────────────────────
async function runCycle() {
  console.log(`\n🕐 ${new Date().toLocaleString()} — Starting cycle...`);
  console.log(`📋 Will post ${CATEGORIES_CONFIG.length} articles (one per category)\n`);
  ...
  for (const catConfig of CATEGORIES_CONFIG) {
    ...
    try {
      ...
      // 4. Upload image
      const imageId = await uploadFeaturedImage(content.focus_keyword, auth);
  ...
  console.log(`\n✅ Cycle complete — ${published}/${CATEGORIES_CONFIG.length} articles published.`);
}
