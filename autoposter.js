...
// ── STEP 4: Upload featured image via Pexels API ────────────────────────────
// Detect politician from title/keyword
function detectPolitician(title, keyword) {
  ...
  const politicians = [
    ...
    { name: 'Nancy Pelosi',    terms: ['pelosi'] },
    { name: 'Mike Pence',      terms: ['pence'] },
    ...
    { name: 'Mitch McConnell', terms: ['mcconnell'] },
    { name: 'Marco Rubio',     terms: ['rubio'] },
    { name: 'Ted Cruz',        terms: ['ted cruz'] },
    { name: 'AOC',             terms: ['aoc', 'ocasio-cortez'] },
  ];
...
// Build smart Pexels search query from article content
function buildImageQuery(articleTitle, keyword, categoryName) {
  const categoryBase = {
    'Politics':      'washington dc government',
    'Business':      'business finance economy',
    'Entertainment': 'entertainment hollywood',
    'Sports':        'sports athlete',
    'Technology':    'technology digital innovation',
    'World News':    'international world news',
    'US News':       'america united states',
  };

  // Check for politician first
  const politician = detectPolitician(articleTitle, keyword);
  if (politician && categoryName === 'Politics') {
    console.log(`👤 Politician detected: ${politician}`);
    return politician;
  }

  // Use first 4 meaningful words from title + category base
  ...
  if (PEXELS_KEY) {
    try {
      ...
      if (photos.length > 0) {
        ...
        console.log(`✅ Pexels: "${pick.alt || query}" by ${pick.photographer}`);
      } else {
        // Try simpler category query if title search returns nothing
        const res2 = await axios.get('https://api.pexels.com/v1/search', {
          headers: { Authorization: PEXELS_KEY },
          params:  { query: keyword, per_page: 10, orientation: 'landscape' },
          timeout: 15000,
        });
        const photos2 = res2.data?.photos || [];
        if (photos2.length > 0) {
          const pick2 = photos2[Math.floor(Math.random() * Math.min(5, photos2.length))];
          imageUrl = pick2.src.large2x || pick2.src.large;
          altText  = `${keyword} - Photo by ${pick2.photographer} on Pexels`;
          console.log(`✅ Pexels fallback: by ${pick2.photographer}`);
        }
      }
    } catch (e) {
      console.warn('⚠️ Pexels API error:', e.message);
    }
  }

  // ── Fallback: Wikimedia category image ───────────────────────────────────
  if (!imageUrl) {
    const wikiFallbacks = {
