...
async function uploadFeaturedImage(keyword, auth) {
  try {
    // Unsplash Source — free random image by keyword, no API key needed
    const imageUrl = `https://source.unsplash.com/1200x630/?${encodeURIComponent(keyword + ' politics usa')}`;
    ...
    const filename   = keyword.replace(/\s+/g, '-').toLowerCase() + '-' + Date.now() + '.jpg';
